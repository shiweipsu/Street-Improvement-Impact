...
memphis_sales <- read_csv("Data/corridor_sales_receipts_final.csv")
...
#set two groups, filter out S Cooper St ,drop the street_name variable 

memphis_sales <- memphis_sales %>% 
  mutate(street_group = case_when(district_name == "Madison Ave" | street_name == "Madison" ~ 1,
                                  district_name == "Broad Ave" | street_name == "Broad" ~ 2))

memphis_sales <-memphis_sales %>% 
  filter(district_name != "S Cooper St")
