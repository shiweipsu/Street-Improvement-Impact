...
lyndale_agg <- agg_index_trend_table(minn_corridor, group = 4, 2008) %>% 
  filter(!is.na(year))

lyndale_agg$business_sd <- NULL

lyndale_agg <- bind_rows(lyndale_agg, minn_agg)

#manually build plot----------------------------

lyndale_agg$Type <- factor(lyndale_agg$Type, 
                             levels = c("improvement", "control", "city"))

 retail <- ggplot(lyndale_agg,aes(x=as.numeric(as.character(year)), 
 ...
 accom <- ggplot(lyndale_agg,aes(x=as.numeric(as.character(year)), 
 ...
 city_lyndale <- plot_grid(retail, accom, align = "h")
