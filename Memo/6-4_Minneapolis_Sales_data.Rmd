...
second_agg_retail_plot <- ggplot(second_agg,aes(x=year, y=CNS07_sd,group=Type,colour=Type,shape=Type))+
  ...
  labs(title = "Retail Sales Comparison: North Second Ave", x="Year",y="Sale Index",
       caption = "Sales tax is indexed to 2011 \n Shaded Area is Construction Period")

second_agg_accom_plot <- ggplot(second_agg,aes(x=year, y=CNS18_sd,group=Type,colour=Type,shape=Type))+
  geom_rect(aes(xmin = 2011, xmax = 2012, ymin = -Inf, ymax = Inf),fill = "darkolivegreen1",linetype=0,alpha = 0.03) +
  geom_line() + 
  geom_point(size=3, fill="white") +
  scale_shape_manual(values=c(22,21))+
