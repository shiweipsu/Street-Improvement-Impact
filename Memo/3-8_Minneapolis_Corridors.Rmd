...
p_load(here, stargazer, sf, tidyr, ggplot2, ggthemes, cowplot, dplyr)
...
riverside_agg_retail <- city_agg_index_trend_plot(riverside_agg_idx, industry = "Retail", corridor_name = "Riverside Ave.", industry_code = "CNS07_sd", construct_year = 2009, end_year = 2010)

riverside_agg_accom <- city_agg_index_trend_plot(riverside_agg_idx, industry = "Accommodations", corridor_name = "Riverside Ave.", industry_code = "CNS18_sd", construct_year = 2009, end_year = 2010)
...
In order to have a fuller understanding of the impact of new cycle infrastructure we also collected sales tax data for the corridors from the state of Minnesota. As mentioned in the introduction, sales taxes are a more sensitive measure of economic impact than employment and is measured more frequently. As such, we have a more sensitive measure of economic impact.
...
```{r riverside_sales, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.asp=.5}
...
###3.1.5 Difference-in-Difference
...
The non-significant results from the *ts_year:pre_post* term gives some support evidence that the construction was not directly responsible for the drop in sales tax receipts for the corridor that the DiD highlighted. Overall, it seems unlikely there is a clear causal relationship between the corridor construction and employment or sales tax effects.
...
franklin_idx_retail <- city_agg_index_trend_plot(franklin_agg, industry = "Retail", industry_code = "CNS07_sd", construct_year = 2011, end_year = 2012, corridor_name = "Franklin Ave. Corridor")

franklin_idx_accom <- city_agg_index_trend_plot(franklin_agg, industry = "Accommodations", industry_code = "CNS18_sd", construct_year = 2011, end_year = 2012, corridor_name = "Frankline Ave. Corridor")
...
franklin_agg_accom_plot <- ggplot(franklin_agg,aes(x=year, y=CNS18_sd,group=Type,colour=Type,shape=Type))+
  ...
  scale_x_continuous(breaks=c(2004,2006,2008,2010,2012,2014,2016)) +
  theme_minimal() +
  labs(title = "Resturaunt Sales Comparison: Franklin Ave", x="Year",y="Sale Index",
       caption = "Sales tax is indexed to 2009 \n Shaded Area is Construction Period")

plot_grid(franklin_agg_retail_plot, franklin_agg_accom_plot, align = "h")
...
The mixed results are also echoed when looking at the sales tax receipts. While there is a marginally significant and positive result for the *pre_post* term showing positive receipt growth post-construction, the *ts_year:pre_post* term is negative and significant for restaurant sales tax receipts. While retail sales do not seem to have been harmed restaurants given this model specification, there is some stronger evidence that the construction had a negative influence on restaurant sales.
...
central_agg_idx_retail <- city_agg_index_trend_plot(central_agg, industry = "Retail", corridor_name = "Central Ave.", industry_code = "CNS07_sd", construct_year = 2012, end_year = 2013)

central_agg_idx_accom <- city_agg_index_trend_plot(central_agg, industry = "Accommodations", corridor_name = "Central Ave.", industry_code = "CNS18_sd", construct_year = 2012, end_year = 2013)
...
###3.3.4 Sales Tax Comparison

Sales tax receipts for Central Ave do not necessarily signal an effect of lane installation in one way or the other. While Central has received more receipts, indexed to 2012, from both retail and restaurants there does not seem to be a clear relationship to the construction date. The LEHD employment data cannot give us a better idea here as it is not possible to pull out restaurants or detailed retail sectors directly. But looking at these trends it does not seem construction of the lane had an immediate impact on sales tax for Central Ave for either industry. 

```{r central_sales_tax, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.asp=.5}
...
###3.3.5 Difference-in-Difference
...
###3.4.5 Difference-in-Difference
...
The DiD specification for sales tax for Lyndale shows a non-significant effect of construction on restaurant sales tax receipts but a positive and significant effect on retail sales. 
...
###3.5.4 Sales Tax Comparison

North 2nd offers an example of a potentially positive affect of construction, at least for retail sales taxes. There is an initial drop in retail receipts during the construction period, as one would hypothesize during a disruptive construction process on a commercial corridor, with an immediate rebound in the following year. Additionally, the corridor sees consistent growth in receipts followed by a spike in receipts around 2014. 

[fix error]

```{r eval=FALSE, fig.asp=.5, fig.width=9, message=FALSE, warning=FALSE, include=FALSE}
...
We were unable to run a difference-in-difference estimate for sales tax on this corridor due to missing data.
...
###3.6.1 Aggregated Trend Analyses

The aggregated trend analyses presented here show  some potentially interesting effects of street improvement for a select set of corridors, but there do not seem to be any clear patterns running in any given direction. At best, we may be able to conclude that the descriptive data offered here does not show any *negative* effect of street improvement on retail or accommodations employment, and, in some cases, may have a positive effect on employment growth. 

A similarly mixed conclusion can be drawn from the sales tax data. In most cases, there is positive sales tax growth post-infrastructure placement, but it is not definitive that the infrastructure itself was responsible for the growth as many corridors were growing before installation. In other words, sales taxes were already trending upwards before, or during, infrastructure conclusion so the effect of the infrastructure is ambiguous. Again, we can probably say with some confidence, though, that the infrastructure placement did not have any major *negative* effects on retail and restaurant sales receipts.

###DiD and ITS Results

Before delving into the overall results of the econometric models it is important to revisit the assumptions that underlie the models and the difficulty in guaranteeing messy observational data adheres to them. For DiD estimates there are two primary assumptions that should be met in order for the conclusions to be valid: first, the parallel trends assumption, this assumption requires that your treatment and control group have similar growth trends before the treatment; and second, the independence assumption, this assumption requires that the treatment and control corridor observations be independent of one another meaning that a change in one corridor does not affect the other. For ITS, having an adequate sample size of observations both pre and post-treatment are necessary for the calculation of proper estimates. This means that for some corridors with only a couple of observations post-treatment likely have biased estimates. Most of the study corridors presented here violate at least one of these assumptions, so the model results should be read critically and as suggestive of infrastructure's real impact even though the exact estimates may be off. That being said, these modeling approaches are some of the best tools researchers have to estimate different policy impacts and offer us better approximations of what's going on than simply aggregate trends analyses.

The DiD results showed a mix, but largely positive, effect of new bike infrastructure on retail and accommodations employment. Two notable corridors showing positive employment growth- Riverside and Franklin Ave- show positive employment growth in either retail or accommodations and each corridor is responsible for greater than 100 additional jobs compared to their control corridors, according to their DiD estimates. Unfortunately, both the Riverside and Franklin Ave corridor ITS estimates were less impressive than their DiD estimates. Riverside's ITS results were non-significant and Franklin's IT results showed a large, negative level change for accommodations employment, but a positive, significant change in the accommodations employment growth rate. 
