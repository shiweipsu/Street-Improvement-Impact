...
plot_grid(riverside_agg_retail_plot, riverside_agg_accom_plot, align = "v")
...
```{r riverside_sales_idx, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.asp=.5}
...
plot_grid(riverside_agg_retail_idx_plot, riverside_agg_idx_accom_plot, align = "v")
...
franklin_agg$Type <- factor(franklin_agg$Type, levels = c("Treatment", "Control: Franklin Ave.", "City"))
...
franklin_idx_retail <- ggplot(franklin_agg, aes(x = year, y = CNS07_sd, 
    ...
    theme_minimal() +
    labs(title = "Retail Employment Comparison:\n Riverside Ave", x="Year",y="Employment Index",
         caption = "Employment is indexed to 2011\n Shaded Area is Construction Period") +
    guides(title = "Street Type")

franklin_idx_accom <- ggplot(franklin_agg, aes(x = year, y = CNS18_sd, 
                                group = Type, colour = Type, shape=Type)) + 
    geom_line()  +
    geom_rect(aes(xmin = as.Date("2011-01-01", "%Y"), 
                  xmax = as.Date("2012-01-01", "%Y"), 
                  ymin = -Inf, ymax = Inf),
              fill = "#adff2f",linetype=0,alpha = 0.03) +
...
ITS models were not run for Franklin due to the data consolidation issue. 
...
central_agg$Type <- factor(central_agg$Type, levels = c("Treatment", "Control: Cedar Ave.", "City"))
...
central_agg_idx_retail <- ggplot(central_agg, aes(x = year, y = CNS07_sd, 
                                group = Type, colour = Type, shape=Type)) + 
    geom_line()  +
    geom_rect(aes(xmin = as.Date("2012-01-01", "%Y-%m-%d"), 
    ...
    theme_minimal() +
    labs(title = "Retail Employment Comparison:\n Riverside Ave", x="Year",y="Employment Index",
         caption = "Employment is indexed to 2012\n Shaded Area is Construction Period") +
    guides(title = "Street Type")

central_agg_idx_accom <- ggplot(central_agg, aes(x = year, y = CNS18_sd, 
                                group = Type, colour = Type, shape=Type)) + 
    geom_line()  +
    geom_rect(aes(xmin = as.Date("2012-01-01", "%Y"), 
                  xmax = as.Date("2013-01-01", "%Y"), 
                  ymin = -Inf, ymax = Inf),
              fill = "#adff2f",linetype=0,alpha = 0.03) +
...
plot_grid(central_agg_retail_plot, central_agg_accom_plot, align = "v")
...
```{r central_sales_tax_idx, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.asp=.5}
...
plot_grid(central_agg_idx_retail_plot, central_agg_idx_accom_plot, align = "v")
...
plot_grid(lyndale_agg_retail_plot, lyndale_agg_accom_plot, align = "v")
...
```{r eval=FALSE, echo=FALSE, fig.asp=.5, fig.width=9, message=FALSE, warning=FALSE}
