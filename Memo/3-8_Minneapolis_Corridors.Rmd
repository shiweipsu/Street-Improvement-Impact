...
riverside <- riverside %>% summarise(TotEmp = sum(C000)/nrow(riverside), RetailEmp = sum(CNS07)/nrow(riverside), AccomEmp = sum(CNS18)/nrow(riverside))
# get employment at block level, divided by nrow()

cedar <- cedar %>% summarise(TotEmp = sum(C000)/nrow(cedar), RetailEmp = sum(CNS07)/nrow(cedar),AccomEmp = sum(CNS18)/nrow(cedar))

minn_lehd_2008 <- minn_lehd %>% filter(year == "2008", CNS07+CNS18 > 0)

quant_df <- data.frame(q_tot_emp = quantile(minn_lehd_2008$C000, probs = p, na.rm = TRUE),
                       q_retail = quantile(minn_lehd_2008$CNS07, probs = p, na.rm = TRUE),
                       q_food_accom = quantile(minn_lehd_2008$CNS18, probs = p, na.rm = TRUE),
                       probs = p)
...
franklin <- minn_corridor %>% filter(Name == "Franklin Ave (improvement)", year == "2010")
minnehaha <- minn_corridor %>% filter(Name == "Franklin Ave (control)", year == "2010")

franklin <- franklin %>% summarise(tot_emp = sum(C000)/nrow(franklin), retail_emp = sum(CNS07)/nrow(franklin), accom_emp = sum(CNS18)/nrow(franklin))

minnehaha <- minnehaha %>% summarise(tot_emp = sum(C000)/nrow(minnehaha), retail_emp = sum(CNS07)/nrow(minnehaha), 
accom_emp = sum(CNS18)/nrow(minnehaha))

minn_lehd_2010 <- minn_lehd %>% filter(year == "2010", CNS07+CNS18 > 0)

quant_df <- data.frame(q_tot_emp = quantile(minn_lehd_2010$C000, probs = p, na.rm = TRUE),
                       q_retail = quantile(minn_lehd_2010$CNS07, probs = p, na.rm = TRUE),
                       q_food_accom = quantile(minn_lehd_2010$CNS18, probs = p, na.rm = TRUE),
                       probs = p)
...
franklin_growth <- minn_growth %>%  
  mutate(year=as.numeric(as.character(year))) %>% 
  filter(Group==2, grouptype=="improvement", year<=2010) 
minnehaha_growth <- minn_growth %>%  
  mutate(year=as.numeric(as.character(year))) %>% 
  filter(Group==2, grouptype=="control", year<=2010) 

t.test(franklin_growth$biz_growth,minnehaha_growth$biz_growth)
t.test(franklin_growth$retail_growth,minnehaha$retail_growth)
t.test(franklin_growth$food_accom_growth,minnehaha$food_accom_growth)
...
In terms of the t-tests both corridors have statistically non-significant differences in "business" employment and their business employment share categories. But because these corridors are actually the same corridor they violate the independence
...
central <- minn_corridor %>% filter(Name == "Central Ave (improvement)", year == "2011")
university <- minn_corridor %>% filter(Name == "University Ave NE (control)", year == "2011")

central <- central %>% summarise(tot_emp = sum(C000)/nrow(central), 
                                 retail_emp = sum(CNS07)/nrow(central),
                                   accom_emp = sum(CNS18)/nrow(central))

university <- university %>% 
  summarise(tot_emp = sum(C000)/nrow(university), 
            retail_emp = sum(CNS07)/nrow(university), 
            accom_emp = sum(CNS18)/nrow(university))

minn_lehd_2011 <- minn_lehd %>% filter(year == "2011", CNS07+CNS18 > 0)

quant_df <- data.frame(q_tot_emp = quantile(minn_lehd_2011$C000, probs = p, na.rm = TRUE),
                       q_retail = quantile(minn_lehd_2011$CNS07, probs = p, na.rm = TRUE),
                       q_food_accom = quantile(minn_lehd_2011$CNS18, probs = p, na.rm = TRUE),
                       probs = p)
...
central_growth <- minn_growth %>%  
  mutate(year=as.numeric(as.character(year))) %>% 
  filter(Group==3, grouptype=="improvement", year<=2011) 
uni_growth <- minn_growth %>%  
  mutate(year=as.numeric(as.character(year))) %>% 
  filter(Group==3, grouptype=="control", year<=2011) 

t.test(central_growth$biz_growth,uni_growth$biz_growth)
t.test(central_growth$retail_growth,uni_growth$retail_growth)
t.test(central_growth$food_accom_growth,uni_growth$food_accom_growth)
```
...
The table in the following shows the summary of above similarity analysis for treatment and control corridor groups. Nine indicators are checked for those corridor groups. Note that all corridors seem comparable though a few lacked any meaningful food/accommodations employment at the block level they generally met other criteria. Note that while Franklin meets these basic criteria it fails the independence assumption since the corridors are different sections of the same street. This means that Franklin is not an appropriate corridor for difference-in-difference testing.  
...
#Define function: city_agg_index_trend_one_control_plot_2
city_agg_index_trend_one_control_plot_2 <- function(df_plot, base, industry, corridor_name, control_corridor, 
...
riverside_agg_retail <- city_agg_index_trend_one_control_plot_2(riverside_agg_idx, base = "Employment Index", industry = "Retail", corridor_name = "Riverside Ave.", control_corridor = "Cedar Ave.", industry_code = "CNS07_sd", construct_year = 2009, end_year = 2010)

riverside_agg_accom <- city_agg_index_trend_one_control_plot_2(riverside_agg_idx, base = "Employment Index", industry = "Accommodations", corridor_name = "Riverside Ave.", control_corridor = "Cedar Ave.", industry_code = "CNS18_sd", construct_year = 2009, end_year = 2010)
...
```{r , echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.asp=1}
...
riverside_agg_retail_plot <- ggplot(riverside_agg,aes(x=year, y=CNS07,group=Type,colour=Type,shape=Type))+
  ...
  theme_minimal() +
  labs(title = "Retail Sales Comparison: \nRiverside Ave", x="Year",y="Sales Tax") +
...
riverside_agg_accom_plot <- ggplot(riverside_agg,aes(x=year, y=CNS18,group=Type,colour=Type,shape=Type))+
  ...
  scale_x_continuous(breaks=c(2004,2006,2008,2010,2012,2014,2016)) +
  theme_minimal() +
  labs(title = "Restaurant Sales Comparison: \nRiverside Ave", x="Year",y="Sales Tax") +
  scale_y_continuous(labels = scales::dollar) +
  theme(legend.position = "none")
...
riverside_agg_retail_idx_plot <- ggplot(riverside_agg_idx,aes(x=year, y=CNS07_sd,group=Type,colour=Type,shape=Type))+
  ...
  scale_x_continuous(breaks=c(2004,2006,2008,2010,2012,2014,2016)) +
  theme_minimal() +
  labs(title = "Retail Sales Comparison: \nRiverside Ave", x="Year",y="Sale Index",
...
prow <- plot_grid(riverside_agg_retail_plot,
                  riverside_agg_accom_plot,
                  riverside_agg_retail_idx_plot,
                  riverside_agg_idx_accom_plot,
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.asp=1.8}
...
qc_1 <- ggplot(riverside_qcew, 
  ...
  theme_minimal() +
    geom_point(size = 3, fill = "white") +
  labs(x = "Quarter", y = "Employment", title = "Retail Employment Comparison \nRiverside Ave ") +
...
qc_2 <- ggplot(riverside_qcew, 
  ...
  theme_minimal() +
    geom_point(size = 3, fill = "white") +
  labs(x = "Quarter", y = "Wages", title = "Retail Total Wages \nRiverside Ave ") +
  guides(shape = guide_legend("Corridor"), colour = guide_legend("Corridor")) +
  scale_y_continuous(labels = scales::dollar) +
    theme(axis.text.x = element_text(angle=45, vjust = 0.5))+
  theme(legend.position = "none")
...
qc_idx1 <- ggplot(riverside_qcew, 
  ...
  scale_x_yearqtr() +
    scale_color_manual(values = c("#00BFC4", "#F8766D")) +
  theme_minimal() +
  labs(x = "Quarter", y = "Indexed Employment", title = "Riverside Ave Retail\n Employment Growth",
       caption = "Data are indexed to the first quarter of 2009") +
  guides(shape = guide_legend("Corridor"), colour = guide_legend("Corridor")) +
    theme(axis.text.x = element_text(angle=45, vjust = 0.5)) +
  theme(legend.position = "none")
...
qc_idx2 <- ggplot(riverside_qcew, 
  ...
  scale_color_manual(values = c("#00BFC4", "#F8766D")) +
  theme_minimal() +
  labs(x = "Quarter", y = "Indexed Wages", title = "Riverside Ave Retail\n Total Wage Change",
       caption = "Data are indexed to the first quarter of 2009") +
  guides(shape = guide_legend("Corridor"), colour = guide_legend("Corridor")) +
    theme(axis.text.x = element_text(angle=45, vjust = 0.5))+
  theme(legend.position = "none")
...
legend_b <- get_legend(qc_1 + theme(legend.position="bottom"))
...
franklin_idx_retail <- city_agg_index_trend_one_control_plot_2(df_plot = franklin_agg, industry = "Retail",
                                                            corridor_name = "Franklin Ave.", control_corridor = "Franklin Ave.", industry_code = "CNS07_sd", construct_year = 2011, end_year = 2012)

franklin_idx_accom <- city_agg_index_trend_one_control_plot_2(df_plot = franklin_agg, industry = "Accommodations", corridor_name = "Franklin Ave.", control_corridor = "Franklin Ave.", industry_code = "CNS18_sd", construct_year = 2011, end_year = 2012)
...
franklin_agg_accom_plot <- ggplot(franklin_agg,aes(x=year, y=CNS18,group=Type,colour=Type,shape=Type))+
  geom_rect(aes(xmin = 2011, xmax = 2012, ymin = -Inf, ymax = Inf),fill = "darkolivegreen1",linetype=0,alpha = 0.03) +
  geom_line() + 
  geom_point(size=3, fill="white") +
  scale_shape_manual(values=c(22,21))+
...
plot_grid(qc_idx_emp_franklin, qc_idx_franklin_wages,  ncol = 1)
...
central_agg_idx_retail <- city_agg_index_trend_one_control_plot_2(df_plot = central_agg, industry = "Retail", corridor_name = "Central Ave.", control_corridor = "University Ave,", industry_code = "CNS07_sd", construct_year = 2012, end_year = 2013)

central_agg_idx_accom <- city_agg_index_trend_one_control_plot_2(df_plot = central_agg, industry = "Accommodations", corridor_name = "Central Ave.", control_corridor = "University Ave,", industry_code = "CNS18_sd", construct_year = 2012, end_year = 2013)
...
```{r central_sales_tax, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.asp=1.0}
...
central_agg_retail_plot <- ggplot(central_agg,aes(x=year, y=CNS07,group=Type,colour=Type,shape=Type))+
  ...
  scale_x_continuous(breaks=c(2004,2006,2008,2010,2012,2014,2016)) +
  theme_minimal() +
  labs(title = "Retail Sales Comparison: \nCentral Ave", x="Year",y="Sales Tax") +
  scale_y_continuous(labels = scales::dollar)
...
central_agg_idx_retail_plot <- ggplot(central_agg_idx,aes(x=year, y=CNS07_sd,group=Type,colour=Type,shape=Type))+
  ...
  scale_x_continuous(breaks=c(2004,2006,2008,2010,2012,2014,2016)) +
  theme_minimal() +
  labs(title = "Retail Sales Comparison: \nCentral Ave", x="Year",y="Sale Index",
...
prow <- plot_grid(central_agg_retail_plot+ theme(legend.position="none"),
                  ...
                  central_agg_idx_retail_plot + theme(legend.position="none"),
...
lyndale_idx_retail <- city_agg_index_trend_one_control_plot_2(df_plot = lyndale_trend,industry = "Retail",corridor_name = "Lyndale Ave. S.", control_corridor = "Grand Ave.",industry_code = "CNS07_sd",construct_year = 2008, end_year = 2009)

lyndale_idx_accom <- city_agg_index_trend_one_control_plot_2(df_plot = lyndale_trend,industry = "Accommodations",corridor_name = "Lyndale Ave. S.", control_corridor = "Grand Ave.",industry_code = "CNS18_sd",construct_year = 2008, end_year = 2009)
...
prow <- plot_grid(lyndale_idx_retail + theme(legend.position="none"),
                  lyndale_idx_accom + theme(legend.position="none"),
                  align = 'vh', hjust = -1, nrow = 1)

legend_b <- get_legend(lyndale_idx_retail + theme(legend.position="bottom"))
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.asp=1.0}
...
lyndale_agg_retail_plot <- ggplot(lyndale_agg,aes(x=year, y=CNS07,group=Type,colour=Type,shape=Type))+
  ...
  scale_x_continuous(breaks=c(2004,2006,2008,2010,2012,2014,2016)) +
  theme_minimal() +
  labs(title = "Retail Sales Comparison: \nLyndale Ave S.", x="Year",y="Sales Tax") +
  scale_y_continuous(labels = scales::dollar)
...
lyndale_agg_idx_retail_plot <- ggplot(lyndale_agg_idx,aes(x=year, y=CNS07_sd,group=Type,colour=Type,shape=Type))+
  ...
  scale_x_continuous(breaks=c(2004,2006,2008,2010,2012,2014,2016)) +
  theme_minimal() +
  labs(title = "Retail Sales Comparison: \nLyndale Ave", x="Year",y="Sale Index",
...
prow <- plot_grid(lyndale_agg_retail_plot + theme(legend.position="none"),
                  ...
                  lyndale_agg_idx_retail_plot + theme(legend.position="none"),
...
north_retail_idx <- city_agg_index_trend_one_control_plot_2(df_plot = north_agg, industry = "Retail", corridor_name = "North 2nd St.", control_corridor = "Broadway Ave.", industry_code = "CNS07_sd", construct_year = 2011, end_year = 2012)

north_accom_idx <- city_agg_index_trend_one_control_plot_2(df_plot = north_agg, industry = "Accommodations", corridor_name = "North 2nd St.", control_corridor = "Broadway Ave.", industry_code = "CNS18_sd", construct_year = 2011, end_year = 2012)
...
prow <- plot_grid(north_retail_idx + theme(legend.position="none"),
                  north_accom_idx + theme(legend.position="none"),
                  align = 'vh', hjust = -1, nrow = 1)

legend_b <- get_legend(north_retail_idx + theme(legend.position="bottom"))
...
```{r north_sales, echo=FALSE, fig.asp=.5, fig.width=9, message=FALSE, warning=FALSE}
...
second_agg_retail_plot <- ggplot(second_agg,aes(x=year, y=CNS07,group=Type,colour=Type,shape=Type))+
  ...
  scale_x_continuous(breaks=c(2004,2006,2008,2010,2012,2014,2016)) +
  theme_minimal() +
  labs(title = "Retail Sales Comparison: \nNorth Second Ave", x="Year",y="Sales Tax") +
  scale_y_continuous(labels = scales::dollar)
...
prow <- plot_grid(second_agg_retail_plot + theme(legend.position="none"),
                  second_agg_idx_retail_plot + theme(legend.position="none"),
                  align = 'vh', hjust = -1, nrow = 1)

legend_b <- get_legend(second_agg_retail_plot + theme(legend.position="bottom"))
