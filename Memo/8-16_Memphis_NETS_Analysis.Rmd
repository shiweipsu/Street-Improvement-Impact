...
# make combined table         
t <- rbind(lehd, sales, qcew, nets1e, nets1s, nets2e, nets2s)
t[t=="NaN"]<-NA
t[t==0]<-NA
...
options(knitr.kable.NA = '-')
  ...
  group_rows("NETS: (employment, type1)",13,16) %>%
  group_rows("NETS: (employment, type2)", 17,20) %>%
  group_rows("NETS: (sales, type1)", 21,24) %>%
  group_rows("NETS: (sales, type2)", 25,28) %>%
  footnote(number = c("Baseline is defined as the average of previous three years before construction year;",
             "Pre-growth rate is defined as average of baseline annual growth rate;",
             "1st year growth rate is defined as the growth rate of the year after construction compared to baseline."))
...
data <- data.frame(c(rep("LEHD\n[employment]",4), rep("Sales\n[sales revenue])",4),rep("QCEW\n[employment]",4),
                     rep("NETS1\n[employment]",4),rep("NETS1\n[sales revenue]",4),rep("NETS2\n[employment]",4),rep("NETS2\n[sales revenue]",4)))
...
t_c <- t[c(1:3,7:9,13)] 
data <- data.frame(c(rep("NETS1\n[employment]",3),rep("NETS1\n[sales revenue]",3),rep("NETS2\n[employment]",3),rep("NETS2\n[sales revenue]",3)))
t_c <- bind_cols(data, t_c)
coln <- c("Data","Area", "Baseline","Pre-Growth", "Post-Growth","Baseline","Pre-Growth", "Post-Growth")
names(t_c)<-coln
