...
ggplot(data=broad_inc, aes(x=year, y=employment/n)) +
  ...
  labs(title="Income Composition Trend Among Corridors", x="Year", y ="Employment per block", 
       caption = "Bike lane is constructed in 2010") +
...
```{r}
start_end_edu <- broad_edu %>% filter(year>2010) %>%
  select(Type, year, ends_with("_perc")) %>% # select variables ends with "_perc"
  filter(complete.cases(.)) %>%
  group_by(Type) %>% 
...
start_end_race <- broad_race %>% filter(year>2010) %>% 
  select(Type, year, ends_with("_perc")) %>% # select variables ends with "_perc"
  filter(complete.cases(.)) %>%
  group_by(Type) %>% 
  summarise_at(vars(ends_with("_perc")), funs(change = (last(.)-first(.))/first(.)/4*100))
...
## additional distributional analysis - race (table)
```{r}
start_end_race <- madison_race %>% filter(year>2011) %>%
  select(Type, year, ends_with("_perc")) %>% # select variables ends with "_perc"
  filter(complete.cases(.)) %>%
  group_by(Type) %>% 
  summarise_at(vars(ends_with("_perc")), funs(change = (last(.)-first(.))/first(.)/3*100))
...
## additional distributional analysis - edu (table)
```{r}
start_end_edu <- madison_edu %>% filter(year >2011) %>% 
  select(Type, year, ends_with("_perc")) %>% # select variables ends with "_perc"
  filter(complete.cases(.)) %>%
  group_by(Type) %>% 
  summarise_at(vars(ends_with("_perc")), funs(change = (last(.)-first(.))/first(.)/3*100))
