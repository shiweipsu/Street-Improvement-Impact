...
p_load(here, RPostgreSQL, sf, ggplot2, directlabels,ggthemes, tidyverse, dbplyr, stargazer,cowplot, lubridate, data.table, readRDS)
...
sf_lehd <- readRDS("Data/sf_lehd.RDS")
sf_corridor <- readRDS("Data/sf_corridor.RDS")
sf_corridor <- st_join(st_buffer(sf_corridor,20),sf_lehd, join=st_intersects)
sf_blocks <- left_join(as.data.frame(sf_corridor), unique(sf_lehd[,c(1,56)]), by = "geoid10") %>% 
  mutate(geometry.x=NULL, geometry=geometry.y, geometry.y=NULL) %>% st_sf


sf_corridor[sf_corridor$fullname =="Polk St",]$type <- "Treatment: Polk"
sf_corridor[sf_corridor$fullname=="Van Ness Ave",]$type <- "Control: Van Ness"
sf_corridor[sf_corridor$fullname =="17th St",]$type <- "Treatment: 17th St"
sf_corridor[sf_corridor$fullname=="18th St",]$type <- "Control: 18th St"
...
sf_corridor <- sf_corridor %>%
  rename(C000 = c000, CNS07 = cns07, CNS08 = cns08, CNS12 = cns12, CNS14 = cns14, CNS15 = cns15,
         ...
         Group=group, BuildStart = buildstart, BuildEnd = buildend, Name = fullname, Type=type)
...
sf_rac <- readRDS("Data/sf_rac.RDS")
sf_corridor <- readRDS("Data/sf_corridor.RDS")
sf_corridor_rac <- st_join(st_buffer(sf_corridor,20),sf_rac, join=st_intersects)

sf_corridor_rac[sf_corridor_rac$fullname =="Van Ness Ave",]$type <- "Control: Van Ness"
sf_corridor_rac[sf_corridor_rac$fullname=="Polk St",]$type <- "Treatment: Polk"
sf_corridor_rac[sf_corridor_rac$fullname =="17th St",]$type <- "Treatment: 17th St"
sf_corridor_rac[sf_corridor_rac$fullname=="18th St",]$type <- "Control: 18th St"
...
nets <- readRDS("Data/nets.RDS") %>% filter(city =="SAN FRANCISCO")
sf_corridor <- readRDS("Data/sf_corridor.RDS")
sf_nets_b <- st_join(nets, unique(sf_blocks[,c(1:5,61)]) %>% st_sf, join=st_intersects, left=F)
sf_nets_bf <- st_join(st_buffer(sf_corridor,20),nets, join=st_intersects)
sf_nets_bf <- left_join(as.data.frame(sf_nets_bf), nets[,c(1,92)], by = "dunsnumber") %>% 
  mutate(geometry.x=NULL, geometry=geometry.y, geometry.y=NULL) %>% st_sf(.)
 ...
 sf_polk_b <- sf_nets_b %>% 
  ...
  separate(key, c("retail", "year"), sep=-3) %>% 
  spread(retail, value) %>% 
  mutate(biz1 = case_when(substr(naics.1,1,3) %in% c(441:454) ~ "Retail",
                               substr(naics.1,1,3) %in% c(722) ~ "Food"),
         year = ifelse(as.numeric(year)<16, paste0("20",year), paste0("19",year)))
 
 sf_polk_bf <- sf_nets_bf %>% 
  filter(group==1) %>% 
  gather(key, value, -c(1:15,94:97)) %>% 
  separate(key, c("retail", "year"), sep=-3) %>% 
  spread(retail, value) %>% 
  mutate(biz2 = case_when((substr(naics.1,1,3) %in% c(443,445,446,448,451,452,453)|substr(naics.1,1,4) %in% c(8121,8123,8129)) ~ "Retail",
                               substr(naics.1,1,4) %in% c(7224,7225) ~ "Food"),
         year = ifelse(as.numeric(year)<16, paste0("20",year), paste0("19",year)))
...
sf_nets_a <- nets_long %>% 
  separate(variable, c("retail", "year"), sep=-3) %>% 
  spread(retail, value) %>% 
...
polk_nets_b <- sf_polk_b %>% 
  group_by(fullname, type, buildstart, buildend, group, year, biz1) %>% 
...
sf_race <- sf_lehd %>% 
  ...
  mutate(white_perc = white/tot_emp*100,
         ...
         other_perc = other/tot_emp*100,
         Type = "City") %>% 
  select(Type, year:other_perc) %>% as.data.frame() %>% select(-geometry)
...
white_plot <- dist_trend_plot(polk_race,demo = "White", demo_code = "white_perc", corridor_name = "Polk Street", cat = "Employment", construct_year = 2009, end_year = 2010, 55, 80)

black_plot <- dist_trend_plot(polk_race,demo = "Black", demo_code = "black_perc", corridor_name = "Polk Street", cat = "Employment", construct_year = 2009, end_year = 2010, 0,25)

ameindian_plot <- dist_trend_plot(polk_race,demo = "American Indian", demo_code = "ameindian_perc", corridor_name = "Polk Street", cat = "Employment",construct_year = 2009, end_year = 2010, 0,25)
...
hawaii_plot <- dist_trend_plot(polk_race,demo = "Native Hawaiian", demo_code = "hawaii_perc", corridor_name = "Polk Street", cat = "Employment", construct_year = 2009, end_year = 2010,0,25)

other_plot <- dist_trend_plot(polk_race,demo = "Two or More Races", demo_code = "other_perc", corridor_name = "Polk Street", cat = "Employment", construct_year = 2009, end_year = 2010,0,25)
...
sf_edu <- sf_lehd %>% 
  ...
  mutate(hisch_under_perc = hisch_under/tot_emp*100,
         ...
         bach_perc = bach/tot_emp*100,
         Type = "City") %>% 
  select(Type, year:bach_perc) %>% as.data.frame(.) %>% select(-geometry)
