...
# load nets
sf_nets <- st_read(dsn = con, query = "select * FROM nets a, sf_corridor b
WHERE ST_Intersects(ST_Buffer(b.geometry, 20), a.geometry);")
...
sf_nets_all <- st_read(dsn = con, query = "select * from nets") %>% filter (city == "SAN FRANCISCO")
sf_nets_a <- sf_nets_all %>% gather(key, value, -c(1:10,89:97)) %>% 
  separate(key, c("retail", "year"), sep=-2) %>% 
  spread(retail, value) %>% 
  mutate(business1 = case_when(substr(naics,1,3) %in% c(441:454) ~ "Retail",
                               substr(naics,1,3) %in% c(722) ~ "Food"),
...
conyear <- first(filter(sf_corridor, Group== 1, Type== "Treatment")$BuildStart)-1 
# find the year before construction

sf_lehd_2008 <- sf_lehd %>% filter(year == conyear, CNS07+ CNS18 > 0)

polk <- sf_corridor %>% filter(Group ==1, Type =="Treatment", year==conyear) %>% employ_ratio_test(.)
...
polk_agg_retail_plot <- agg_index_trend_plot(polk_sale, industry = "Retail", corridor_name = "Polk St.",industry_code = "CNS07_sd", construct_year = 2009, end_year = 2010)
polk_agg_food_plot <- agg_index_trend_plot(polk_sale, industry = "Food/Accommodations", corridor_name = "Polk St.",industry_code = "CNS18_sd", construct_year = 2009, end_year = 2010)
...
polk_nets1 <- sf_polk %>% 
  group_by(fullname, type, buildstart, buildend, group, year, business1) %>% 
  summarise(emp = sum(emp), sales= sum(sales), n=n(),
            emp_pest = emp/n, sales_pest = sales/n) %>% drop_na(.)
sf_nets1 <- sf_nets1 %>% 
  mutate(fullname="city", type="City", buildstart=0, buildend=0, group=1) %>% select(fullname:group, year:geometry)

polk_nets1 <- rbind(polk_nets1, sf_nets1) %>% select(-geometry) %>% mutate(geometry=0)

# employment plot
# abousolute employment
polk_nets1_emp <- polk_nets1 %>% select(-sales, -n, -emp_pest, -sales_pest) %>% spread(key = business1, emp) %>% 
...
polk_nets1_emp_pest <- polk_nets1 %>% select(-sales, -n, -emp, -sales_pest) %>% spread(key = business1, emp_pest) %>% 
...
# sales plot
# abousolute sales
polk_nets1_sales <- polk_nets1 %>% select(-emp, -n, -emp_pest, -sales_pest) %>% spread(key = business1, sales) %>% 
  rename(CNS07 = Retail, CNS18 = Food, Group=group, Type= type)
...
polk_nets1_sales_pest <- polk_nets1 %>% select(-sales, -n, -emp, -emp_pest) %>% spread(key = business1, sales_pest) %>% 
  rename(CNS07_pest = Retail, CNS18_pest = Food, Group=group, Type= type)
...
# one types of industry categories similar to LEHD
polk_nets2 <- sf_polk %>% 
  group_by(fullname, type, buildstart, buildend, group, year, business2) %>% 
  summarise(emp = sum(emp), sales= sum(sales), n=n(),
            emp_pest = emp/n, sales_pest = sales/n) %>% drop_na(.)
sf_nets2 <- sf_nets2 %>% 
  mutate(fullname="city", type="City", buildstart=0, buildend=0, group=1) %>% select(fullname:group, year:geometry)

polk_nets2 <- rbind(polk_nets2, sf_nets2) %>% select(-geometry) %>% mutate(geometry=0)

# employment plot
# abousolute employment
polk_nets2_emp <- polk_nets2 %>% select(-sales, -n, -emp_pest, -sales_pest) %>% spread(key = business2, emp) %>% 
  rename(CNS07 = Retail, CNS18 = Food, Group=group, Type= type)
...
# employment per establishment
polk_nets2_emp_pest <- polk_nets2 %>% select(-sales, -n, -emp, -sales_pest) %>% spread(key = business2, emp_pest) %>% 
  rename(CNS07_pest = Retail, CNS18_pest = Food, Group=group, Type= type)
...
# sales plot
# abousolute sales
polk_nets2_sales <- polk_nets2 %>% select(-emp, -n, -emp_pest, -sales_pest) %>% spread(key = business2, sales) %>% 
  rename(CNS07 = Retail, CNS18 = Food, Group=group, Type= type)
...
# sales per establishment
polk_nets2_sales_pest <- polk_nets2 %>% select(-sales, -n, -emp, -emp_pest) %>% spread(key = business2, sales_pest) %>% 
  rename(CNS07_pest = Retail, CNS18_pest = Food, Group=group, Type= type)
...
# DID
#employment
polk_nets1_emp$Type <- relevel(polk_nets1_emp$Type,ref = "Treatment")
polk_nets2_emp$Type <- relevel(polk_nets2_emp$Type,ref = "Treatment")

polk_nets1_emp_did <- did_agg_analysis(filter(polk_nets1_emp, Type!="City" & year>2003), group = 1, endyear = 2010)
polk_nets2_emp_did <- did_agg_analysis(filter(polk_nets2_emp, Type!="City" & year>2003), group = 1, endyear = 2010)
stargazer(polk_nets1_emp_did[[1]], polk_nets2_emp_did[[1]], polk_nets1_emp_did[[2]], polk_nets2_emp_did[[2]], 
          title = "Polk Ave. Corridor Difference-in-Difference Estimates", 
          column.labels  = c("Type 1", "Type 2", "Type 1","Type 2"), 
          type = "text", dep.var.labels = c("Retail Emp.", "Food Emp."), model.numbers = F)

# sales
polk_nets1_sales$Type <- relevel(polk_nets1_sales$Type,ref = "Treatment")
polk_nets2_sales$Type <- relevel(polk_nets2_sales$Type,ref = "Treatment")

polk_nets1_sales_did <- did_agg_analysis(filter(polk_nets1_sales, Type!="City" & year>2003), group = 1, endyear = 2010)
polk_nets2_sales_did <- did_agg_analysis(filter(polk_nets2_sales, Type!="City" & year>2003), group = 1, endyear = 2010)
stargazer(polk_nets1_sales_did[[1]], polk_nets2_sales_did[[1]], polk_nets1_sales_did[[2]], polk_nets2_sales_did[[2]], 
          title = "Polk Ave. Corridor Difference-in-Difference Estimates", 
          column.labels  = c("Type 1", "Type 2", "Type 1","Type 2"), 
          type = "text", dep.var.labels = c("Retail Sales", "Food Sales"), model.numbers = F)

# ITS
polk_nets1_emp_its <- agg_its_analysis(polk_nets1_emp, group = 1, endyear = 2010)
polk_nets2_emp_its <- agg_its_analysis(polk_nets2_emp, group = 1, endyear = 2010)
stargazer(polk_nets1_emp_its[[1]], polk_nets2_emp_its[[1]], polk_nets1_emp_its[[2]], polk_nets2_emp_its[[2]], 
          title = "Polk Ave. Corridor Difference-in-Difference Estimates", 
          column.labels  = c("Type 1", "Type 2", "Type 1","Type 2"), 
          type = "text", dep.var.labels = c("Retail Emp.", "Food Emp."), model.numbers = F)
...
polk_nets2_sales_its <- agg_its_analysis(polk_nets2_sales, group = 1, endyear = 2010)
...
```{r echo=FALSE, fig.asp=0.5, fig.width=9.5, message=FALSE, warning=FALSE}
polk_inc <- sf_corridor %>% as.data.frame() %>% filter(Group == 1) %>% 
  mutate(Type = ifelse(Type == "Treatment", "Treatment: Polk", "Control: Van Ness"),
    Type = factor(Type, levels = c("Treatment: Polk", "Control: Van Ness"))) %>% 
  group_by(Type, year) %>% 
...
col_plot <- dist_trend_plot(polk_edu,demo = "College", demo_code = "col_perc", corridor_name = "Polk Street", cat = "Employment", construct_year = 2009, end_year = 2010, 10, 25)
...
hisch_under_plot <- dist_trend_plot(polk_edu,demo = "Less Than High School", demo_code = "hisch_under_perc", corridor_name = "Polk Street", cat = "Employment", construct_year = 2009, end_year = 2010, 5, 15)

hisch_plot <- dist_trend_plot(polk_edu,demo = "High School", demo_code = "hisch_perc", corridor_name = "Polk Street", cat = "Employment", construct_year = 2009, end_year = 2010, 8, 18)

col_plot <- dist_trend_plot(polk_edu,demo = "College", demo_code = "col_perc", corridor_name = "Polk Street", cat = "Employment", construct_year = 2009, end_year = 2010, 15, 25)

bach_plot <- dist_trend_plot(polk_edu,demo = "Bachelor or Above", demo_code = "bach_perc", corridor_name = "Polk Street", cat = "Employment", construct_year = 2009, end_year = 2010,29,39)
