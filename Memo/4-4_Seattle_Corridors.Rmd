...
Our first corridor group consists of the 2nd and 4th Ave corridors in downtown Seattle. The other control corridor, 1st Ave, is exclueded because half of the blocks overlap with the treatment corridor, which would cause serious assumption violation for later data analysis.
...
firstave <- seattle_corridor %>% filter(Group==1, Name=="1st Ave", year==conyear) %>% employ_ratio_test(.)
...
firstave %>% summarise(TotEmp = sum(C000)/nrow(forthave), RetailEmp = sum(CNS07)/nrow(firstave),
                                     AccomEmp = sum(CNS18)/nrow(firstave)) %>% as.data.frame
...
t.test(secondave$Business, firstave$Business)
t.test(secondave$Retail, firstave$Retail)
t.test(secondave$Food_Accom, firstave$Food_Accom)
t.test(secondave$ratio1, firstave$ratio1)
t.test(secondave$ratio2, firstave$ratio2)
...
firstave_growth <- seattle_growth %>%  
  mutate(year=as.numeric(as.character(year))) %>% 
  filter(Group==1, Name=="1st Ave", year<=conyear) 
...
t.test(secondave_growth$biz_growth,firstave_growth$biz_growth)
t.test(secondave_growth$retail_growth,firstave_growth$retail_growth)
t.test(secondave_growth$food_accom_growth,firstave_growth$food_accom_growth)
```
...
agg_retail_trend_plot <- agg_trend_plot(secondave_agg, industry = "Retail", industry_code = "CNS07", corridor_name = "2nd Ave.", construct_year = 2014, end_year = 2015)
agg_food_trend_plot <- agg_trend_plot(secondave_agg, industry = "Food/Accommodataion",industry_code = "CNS18", corridor_name = "2nd Ave.", construct_year = 2014, end_year = 2015)
...
agg_retail_trend_plot <- agg_trend_plot(brwy_agg, industry = "Retail", industry_code = "CNS07",corridor_name = "Broadway", construct_year = 2014, end_year = 2015)
agg_food_trend_plot <- agg_trend_plot(brwy_agg, industry = "Food/Accomodation", industry_code = "CNS18", corridor_name = "Broadway", construct_year = 2014, end_year = 2015)
...
Table 3: Roosvelt Way, University Way, and 45th St Corridor Employment Percentiles
...
agg_retail_trend_plot <- agg_trend_plot(rsvt_agg, industry = "Retail",industry_code = "CNS07", corridor_name = "Rossevelt Way.", construct_year = 2016, end_year = 2017)
agg_food_trend_plot <- agg_trend_plot(rsvt_agg, industry = "Food/Acommodation",industry_code = "CNS18", corridor_name = "Roosevelt Way", construct_year = 2016, end_year = 2017)
...
secondave_agg_retail_plot <- agg_index_trend_plot(secondave_agg, industry = "Retail", corridor_name = "2nd Ave.",industry_code = "CNS07_sd", construct_year = 2014, end_year = 2015)
secondave_agg_food_plot <- agg_index_trend_plot(secondave_agg, industry = "Food/Accommodations", corridor_name = "2nd Ave.",industry_code = "CNS18_sd", construct_year = 2014, end_year = 2015)

plot_grid(secondave_agg_retail_plot,secondave_agg_food_plot)
...
secondave_agg_retail_plot <- agg_index_trend_plot(second, industry = "Retail", corridor_name = "2nd Ave.",industry_code = "CNS07_sd", construct_year = 2014, end_year = 2015)
secondave_agg_food_plot <- agg_index_trend_plot(second, industry = "Food/Accommodations", corridor_name = "2nd Ave.",industry_code = "CNS18_sd", construct_year = 2014, end_year = 2015)

prow <- plot_grid(secondave_agg_retail_plot + theme(legend.position="none"),
                  secondave_agg_food_plot + theme(legend.position="none"),
                  align = 'vh', hjust = -1, nrow = 1)

legend_b <- get_legend(secondave_agg_retail_plot + theme(legend.position="bottom"))
...
stargazer(secondave_its_agg[[1]],secondave_its_agg[[2]],secondave_its_agg[[3]],type = "text", title = "Second Avenue Corridor Interrupted Time Series Estimates", column.labels  = c("Retail Emp.", "Accomodations Emp.", "'Business' Emp."))
...
#### LEHD
The bike facility installation has taken place in 2013. The business employment continues to growth after two-year bike lane installation. However, it shows similar growth pattern with control corridors, if taking into the account of average of the two. There is a significant drop of retail employment after bike lane installation in Broadway corridor, but the control corridors continues to grow. Although it starts to recover after one year, there is still caveats that bike lane might negatively affect retail services right after the construction period. Food/accomnodation employment in improvement corridor jumps greatly after one-year bike lane installation. The two other corridor still maintains the similar trend as below. This indicates the potential positive effect of bike lane on food/accomnodation service.

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=9.5, fig.asp=0.5}

brwy_agg <- agg_index_trend_table(seattle_corridor, group = 2, construct_year = 2013)

brwy_agg_retail_plot <- agg_index_trend_plot(brwy_agg, industry = "Retail", corridor_name = "Broadway",industry_code = "CNS07_sd", construct_year = 2013, end_year = 2014)
brwy_agg_food_plot <- agg_index_trend_plot(brwy_agg, industry = "Food/Accommodations", corridor_name = "Broadway",industry_code = "CNS18_sd", construct_year = 2013, end_year = 2014)

plot_grid(brwy_agg_retail_plot,brwy_agg_food_plot)

```

#### QCEW Data
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=9.5, fig.asp=0.5}
brwy <- agg_index_trend_table(seattle_qcew %>% mutate(geometry=0), group=2, construct_year = 2013) %>% 
  mutate(Type=as.factor(Type))

brwy[brwy==0] <- NA
brwy_agg_retail_plot <- agg_index_trend_plot(brwy[!is.na(brwy$CNS07_sd),], industry = "Retail", corridor_name = "Broadway",industry_code = "CNS07_sd", construct_year = 2013, end_year = 2014) 
brwy_agg_food_plot <- agg_index_trend_plot(brwy, industry = "Food/Accommodations", corridor_name = "Broadway",industry_code = "CNS18_sd", construct_year = 2013, end_year = 2014)

prow <- plot_grid(brwy_agg_retail_plot + theme(legend.position="none"),
                  brwy_agg_food_plot + theme(legend.position="none"),
                  align = 'vh', hjust = -1, nrow = 1)

legend_b <- get_legend(brwy_agg_retail_plot + theme(legend.position="bottom"))
...
stargazer(broadway_did_agg[[1]],broadway_did_agg[[2]],broadway_did_agg[[3]],type = "text", title = "Broadway Corridor Difference-in-Difference Estimates", column.labels  = c("Retail Emp.", "Accomodations Emp.", "'Business' Emp."))

```

Although the trend graph shows negative effect of bike lane installation on retail employment, the DID estimators for retail employment is not statistically significant, which indicates the effect of treatment is not apparent. The big jump of food/accommodation employment in Broadway treatment corridor is consistent with the DID estimator, that a statistically significant positive effect of new bicycle infrastructure replacement on food/accommodation employment.
...
stargazer(broadway_its_agg[[1]],broadway_its_agg[[2]],broadway_its_agg[[3]],type = "text", title = "Broadway Corridor Interrupted Time Series Estimates", column.labels  = c("Retail Emp.", "Accomodations Emp.", "'Business' Emp."))

```

The ITS estimators for all three models are statistically insignificant, which indicates the effect of treatment is not apparent. Although there is a big jump of food/accommodation employment in Broadway treatment corridor, the causal inference does not exhibit a statistically significant effect of new bicycle infrastructure replacement on employment according to ITS analysis.
...
#### QCEW
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=9.5, fig.asp=0.5}
roos <- agg_index_trend_table(seattle_qcew %>% mutate(geometry=0), group=3, construct_year = 2016) %>% 
  mutate(Type=as.factor(Type))

roos[roos==0] <- NA
roos_agg_retail_plot <- agg_index_trend_plot(roos[!is.na(roos$CNS07_sd),], industry = "Retail", corridor_name = "Roosevelt",industry_code = "CNS07_sd", construct_year = 2016, end_year = 2017) 
roos_agg_food_plot <- agg_index_trend_plot(roos, industry = "Food/Accommodations", corridor_name = "Roosevelt",industry_code = "CNS18_sd", construct_year = 2016, end_year = 2017)

prow <- plot_grid(roos_agg_retail_plot + theme(legend.position="none"),
                  roos_agg_food_plot + theme(legend.position="none"),
                  align = 'vh', hjust = -1, nrow = 1)

legend_b <- get_legend(roos_agg_retail_plot + theme(legend.position="bottom"))
