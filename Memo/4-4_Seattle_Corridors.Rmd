...
agg_trend_plot(secondave_agg, industry = "business", corridor_name = "2nd Ave.", construct_year = 2014, end_year = 2015)
agg_trend_plot(secondave_agg, industry = "CNS07", corridor_name = "2nd Ave.", construct_year = 2014, end_year = 2015)
agg_trend_plot(secondave_agg, industry = "CNS18", corridor_name = "2nd Ave.", construct_year = 2014, end_year = 2015)
```
...
The bike facility installation has just taken place in 2014. The newest employment data for LEHD is in 2015. Therefore,although the employment increases after bike lane installation, there is not much data to support the after-intervention trend.
...
Food and accomodation employment trend is similar to business trend presend above, and the t-test verify the similarity as well. Again, the employment increase after bike lane installation needs more validation according to recent data.
...
agg_trend_plot(brwy_agg, industry = "business", corridor_name = "Broadway", construct_year = 2014, end_year = 2015)
agg_trend_plot(brwy_agg, industry = "CNS07", corridor_name = "Broadway", construct_year = 2014, end_year = 2015)
agg_trend_plot(brwy_agg, industry = "CNS18", corridor_name = "Broadway", construct_year = 2014, end_year = 2015)

```

### Corridor employment amount
The following table shows total, retail, and accommodations employment for Broadway, 15th Ave, and E Olive Way corridors, and the city based percentile rank of the corridors. Broadyway has muach more employment than the other comparison corridors. However, with respect to our interested business employment, the corridors are at similar level. 15th Ave E has slight higher percentiles in both retail and food/accomodation sectors than the improvement corridor, while E Olive Way has slightly less food/accomodation employment with very small amount of retail employment.   
...
With respect to employment number metrics, the t-test returned non-significant results meaning that there is not a statistically significant difference in the mean employment levels between the two corridors. However, the "business share" metrics indicate significant differnce between improvement corridor and 15th Ave. The large share of other service jobs in improvement corridor might influence the impact of bike facility, since the might be comfounding effect from other services to retail employment rather than bike lane installation. But given the similarity in business employment amount, the 15th Ave E corridors are still comparable and are appropriate for continued testing. 

Control corridor 2: E Olive Way:
According to t-test result, although there is significant difference in retail employments between improvement corridor and E Olive Way, the general business vatalitiy are similar, after taking into account of food/accomodation sector.In terms of "business share" metrics, one of the metric (taking into smaller scale of service employment) shows no different in business share between two corridors. We conclude E Olive Way is comparble and appropriate for continued testing.

### Employment trend comparison

The total business employment trend over this recent decade is similar for improvement corridor and 15th Ave. Although E Olive Way corridor experienced dramatic changes in 2007, 2011 and 2012, the overall trend before 2013, the bike lane installation year, is geneally growing, which is similar to the improvement corridors. The non-significance t-test result also verify this.

The bike facility installation has taken place in 2013. The business employment continues to growth after two-years bike lane installation. However, it shows similar growth pattern with controll corridors, if taking into the account of average of the two.
...
Retail employment trend is very similar for improvement corridor and 15th Ave E corridor before the constrution of bike lane. However, the second control corridor, E Olive Way, experienced much more flutuation before 2013, which is unlike the improvement corridor. 

There is a significant drop of retail employment after bike lane installation in Broadway corridor, but the control corridors constinues to grow. Although it starts to recover after one year, there is still caveats that bike lane might negatively affect retail services.
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
The growth trend of food/accomodation service sector are almost parallel across three corridors. The t-test verify this as well.

Food/accomodation employment in improvement corridor jumps greatly after one-year bike lane installation. The two other corridor still maintains the similar trend as below. This indicates the potential positive effect of bike lane on food/accomodation service.
...
agg_trend_plot(rsvt_agg, industry = "business", corridor_name = "Roosevelt Way", construct_year = 2014, end_year = 2015)
agg_trend_plot(rsvt_agg, industry = "CNS07", corridor_name = "Rossevelt Way.", construct_year = 2014, end_year = 2015)
agg_trend_plot(rsvt_agg, industry = "CNS18", corridor_name = "Roosevelt Way", construct_year = 2014, end_year = 2015)
```

### Corridor employment amount
The following table shows total, retail, and accommodations employment for BRoosvelt Way, University Way, and 45th St corridors, and the city based percentile rank of the corridors. Compared with improvement corridor, Roosevelt Way, Univerisity Way corriodor has similar amount of retail employment, while they are both between 85-90 percentile. But Univerisity Way has more food & accomodation service jobs than Roosevelt Way. Although 45th St has similar food and accomodation employment as Roosevelt Way, it has less retail jobs. In general, both the two control corridors are comparable with Roosevelt Way to some extent.  
...
With respect to employment number metrics, Univerisity Way has similar business and retail employment jobs, but it has significant more food/accomodation employment than the improvement corridor. It arise the caveat that the impact of bike lane on food/accomodation factor might be different between the two corridors. Both "business share" metrics t-tests indicate similarity between the two corridors. Although the food/accomodation employment test shows differentce between the two corridors, they are still comparable in terms of all other metrics, and are appropriate for continued testing. 
...
The installation of bike lane on Roosevelt Way is in 2006, where the most recent LEHD data is 2015. Therefore, we cannot compare the trend after bike lane installationg using LEHD data. Other data sources are needed to investigate recent trend.

The total business employment trend for three corridors over this recent decade are growing with some fluctuation. The business employment in improvement corridor generally increases beside during the economic recession period. NE 45th St also increase gentlely over time with slight decrease in 2005 and 2014. University Way has an incrase spike in 2012 and 2013, but descrese after that. The t-test indicates the two control corridors are similar with improvement corridor. 

```{r echo=FALSE, message=FALSE, warning=FALSE}

rsvt_agg <- agg_index_trend_table(seattle_corridor, group = 3, construct_year = 2015)
rsvt_agg_plot <- agg_index_trend_plot(rsvt_agg, industry = "Business", corridor_name = "Roosevelt",industry_code = "business_sd", construct_year = 2016, end_year = 2017)


```


Retail employment trend is very similar for improvement corridor and 45th St corridor before, while University Way experience continuing growth in retail employmetn before 2012 and started to decrease after that. But in general, the the two corridors are similar to improvement corridor based on t-test.

```{r echo=FALSE, message=FALSE, warning=FALSE}

rsvt_agg_plot <- agg_index_trend_plot(rsvt_agg, industry = "Retail", corridor_name = "Roosevelt",industry_code = "CNS07_sd", construct_year = 2016, end_year = 2017)


```

Food and accomodation employment trend is more fluctuated. Imporvement corridor has an apparent recession between 2009-2011, while 45th St didn't change much besides the spike in 2010. University Way is the most fluctuated corridor. However, taking into account of the average trend over this decade, the two control corridors are similar to improvement corridor.

```{r echo=FALSE, message=FALSE, warning=FALSE}

rsvt_agg_plot <- agg_index_trend_plot(rsvt_agg, industry = "Food & Accommodations", corridor_name = "Roosevelt",industry_code = "CNS18_sd", construct_year = 2016, end_year = 2017)
...
```{r}
secondave_did <- did_analysis(seattle_corridor,group = 1, 2014)
stargazer(secondave_did[[1]],secondave_did[[2]],secondave_did[[3]],type = "text", title = "Second Avenue Corridor Difference-in-Difference Estimates", column.labels  = c("Retail Emp.", "Accomodations Emp.", "'Business' Emp."))
...
## Broadway corridor
```{r}
broadway_did <- did_analysis(seattle_corridor,group = 2, 2013)
stargazer(broadway_did[[1]],broadway_did[[2]],broadway_did[[3]],type = "text", title = "Second Avenue Corridor Difference-in-Difference Estimates", column.labels  = c("Retail Emp.", "Accomodations Emp.", "'Business' Emp."))
...
Similarly, the DID estimators for all three models are statistically insignificant, which indicates the effect of treatment is not apparent. Although there is a big jump of food/accomotion employment in Broadway treatment corridor, the causal inference does not exhibit a statistically significant effect of new bicycle infrastructure replacement on employment.

## Roosevelt Way
The Roosevelt Way corridor experienced bike lane construction in 2016, one year later than the newest LEHD data. Thus, we cannot conduct DID analysis because there is no after treatment data points for the corridors. 


# Interupted time series
Interrupted time series is an econometric technique that estimates how street improvements impact corridor economic vitality from a longitudinal perspective. If the treatment has a causal impact, the post-intervention series will have a different level or slope than the pre-intervention series. In our research, interrupted-time series will be used to distinguish differences in growth before and after a specific time break point where the installation of new bicycled facility happens. The estimation of pre-post parameter indicates the impact of interruption on change of intersted outcome variable, while the estimation of  


## Second Avenue corridor
```{r}
secondave_its <- its_analysis(seattle_corridor,group = 1, 2014)
stargazer(secondave_its[[1]],secondave_its[[2]],secondave_its[[3]],type = "text", title = "Second Avenue Corridor Difference-in-Difference Estimates", column.labels  = c("Retail Emp.", "Accomodations Emp.", "'Business' Emp."))
...
## Broadway corridor
```{r}
broadway_its <- its_analysis(seattle_corridor,group = 2, 2013)
stargazer(broadway_its[[1]],broadway_its[[2]],broadway_its[[3]],type = "text", title = "Second Avenue Corridor Difference-in-Difference Estimates", column.labels  = c("Retail Emp.", "Accomodations Emp.", "'Business' Emp."))
...
```{r}
roosevelt_its <- its_analysis(seattle_corridor,group = 3, 2016)
stargazer(roosevelt_its[[1]],roosevelt_its[[2]],roosevelt_its[[3]],type = "text", title = "Second Avenue Corridor Difference-in-Difference Estimates", column.labels  = c("Retail Emp.", "Accomodations Emp.", "'Business' Emp."))
