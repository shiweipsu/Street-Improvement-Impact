...
# Defining function: agg_trend_table_qcew, agg_index_trend_table
...
agg_index_trend_table <- function(df, group, construct_year) {
       ...
       summarise(emp_food_b = mean(emp_food),
                 emp_retail_b = mean(emp_retail),
                 wage_food_b = mean(wage_food),
                 wage_retail_b = mean(wage_retail),
                 estb_food_b = mean(estb_food),
                 estb_retail_b = mean(estb_retail))
  
  
       df_plot <- df_agg %>%  
       left_join(base_year, by = "Type") %>%
       mutate(emp_food_sd = emp_food/emp_food_b*100,
              emp_retail_sd = emp_retail/emp_retail_b*100,
              wage_food_sd = wage_food/wage_food_b*100,
              wage_retail_sd = wage_retail/wage_retail_b*100,
              estb_food_sd = estb_food/estb_food_b*100,
              estb_retail_sd = estb_retail/estb_retail_b*100) %>%
       select(Type, year = year, emp_food_sd, emp_retail_sd,
              wage_food_sd, wage_retail_sd, estb_food_sd, estb_retail_sd)
...
broad_agg_idx <- agg_index_trend_table(emphis_qcew, group = 2, construct_year = 2010)
broad_agg <- bind_rows(broad_agg_idx, broad_agg)

broad_agg_retail_plot <- agg_trend_plot_emp(broad_agg, industry = "Retail", industry_code = "emp_retail",
                                            corridor_name = "Broad Avenue",
                                            construct_year = 2010, end_year = 2011,
                                            index = "")

broad_agg_food_plot <- agg_trend_plot_emp(broad_agg, industry = "Food",industry_code = "emp_food",
                                          corridor_name = "Broad Avenue",
                                          construct_year = 2010, end_year = 2011,
                                          index = "")

broad_agg_retail_idx_plot <- agg_trend_plot_emp(broad_agg, industry = "Retail", industry_code = "emp_retail",
                                            corridor_name = "Broad Avenue",
                                            construct_year = 2010, end_year = 2011,
                                            index = "employment is indexed to the baseline years (3 years pre-construction)")

broad_agg_idx_food_plot <- agg_trend_plot_emp(broad_agg, industry = "Food",industry_code = "emp_food",
                                          corridor_name = "Broad Avenue",
                                          construct_year = 2010, end_year = 2011,
                                          index = "employment is indexed to the baseline years (3 years pre-construction)")

prow <- plot_grid(broad_agg_retail_plot + theme(legend.position="none"),
                  broad_agg_food_plot + theme(legend.position="none"),
                  broad_agg_retail_idx_plot + theme(legend.position="none"),
                  broad_agg_idx_food_plot + theme(legend.position="none"),
