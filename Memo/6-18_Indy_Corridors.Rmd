...
Starting in 1999, Indianapolis began the planning process for the Indianapolis Cultural Trail, an 8-mile long pedestrian and bike trail connecting six historic district in the city. The trail included many street improvements including, but not limited to, protected bike lanes, pedestrian improvements and public art installations. Indianapolis Cultural Trail, Inc., the non-profit managing the trail, completed construction in 2013. In addition to the trail, Indianapolis Cultural Trail, Inc. also runs Pacers Bikeshare. The construction of the trail and bikesharing have dramatically changed the transportation choices in downtown Indianapolis.  
...
Comparing retail and accommodations related employment, we find the Meridian St Corridor has much more employment than the Virginia Ave corridor. The other two comparison corridors, Prospect and Shelby, have similar levels of employment, though, the Virginia Ave corridor has relatively less retail employment. 
...
We also performed a series of t-tests in order to determine whether the average employment levels per block between the treatment and control corridors are statistically significantly different. A statistically significant result here would suggest that the corridors are not necessarily comparable. The Meridian corridor mean 'business' (the sum of retail and accommodations employment) and food employment per block is significantly different than the Virginia corridor blocks, according to our t-tests. Additionally, we performed a second set of t-tests on the business/service employment ratios between the two corridors. In this case, all of the comparison corridors t-tests came back non-significant indicating the corridors have a similar structure of 'business' versus service jobs even as the overall number of jobs per corridor differs.
...
Indexing the employment figures allows us to examine the change in the growth of employment on the same scale. For Virginia Ave, indexing employment to 2011, the beginning of construction, shows us some interesting patterns before and after installation. The retail plot shows that Virginia roughly followed the patterns of its comparator corridors with positive employment growth,  collapse during the recession and then rebounding in the late 2000s/early 2010s. The treatment corridor does not track any particular comparator corridor that closely but it is much closer to Meridian, and the city as a whole, post-construction than the others.

The indexed accommodations employment chart shows a much tighter relationship among the corridors and the city. This should be expected given the relatively small amounts of accommodations employment across the corridors, but Virginia Ave's clear, consistent growth starting after 2007 is tracked only by Prospect. 
...
va_index <- va_index %>% mutate(Type = if_else(Type == "city", "City", Type))

va_index$Type <- factor(va_index$Type, 
                        levels = c("Treatment","City", 
                                   "Control: Meridian",
                                   "Control: Prospect",
                                   "Control: Shelby"))

construct_year <-  as.Date("2011-01-01","%Y-%m-%d")
end_year <- as.Date("2013-01-01","%Y-%m-%d")


va_city_idx_retail_plot <- ggplot(va_index, aes(x = as.Date(year, "%Y"), y = CNS07_sd, 
    ...
    theme_minimal() +
    labs(title = "Retail Employment Comparison:\n Virginia Ave.", x="Year",y="Employment Index",
         caption = glue("Employment is indexed to 2011\n Shaded Area is Construction Period")) +
    guides(title = "Street Type")

va_city_idx_accom_plot <- ggplot(va_index, aes(x = as.Date(year, "%Y"), y = CNS18_sd, 
                                group = Type, colour = Type, shape=Type)) + 
    geom_rect(aes(xmin = as.Date("2011-01-01","%Y-%m-%d"), 
                  xmax = as.Date("2013-01-01","%Y-%m-%d"), 
                  ymin = -Inf, ymax = Inf),
    ...
    scale_x_date(date_breaks = "3 years", date_labels = "%Y") +
    theme_minimal() +
    labs(title = "Accommodations Employment Comparison:\n Virginia Ave.", x="Year",y="Employment Index",
         caption = glue("Employment is indexed to 2011\n Shaded Area is Construction Period")) +
    guides(title = "Street Type")

plot_grid(va_city_idx_retail_plot, va_city_idx_accom_plot, ncol = 1)
```
...
Across all three models both the pre-post term (the term that signals a potential level change in your unit of interest) and the time series-pre_post term (the term that signals a potential slope change in your unit of interest) are non-significant. The time series alone term for accommodations and the combined 'business' variable are significant and positive signaling a general positive growth trend over time that is not related to the installation of the new cycling infrastructure. 


####  Sales Tax

The sales tax ITS model was also non-significant given our specification with a marginally significant time series term signaling the effect of a general positive growth trend above and beyond the placement of new infrastructure. 
...
The employment trend graphs of two corridors are presented below. In terms of retail employment, the two corridors  experienced a significant employment drop around 2007 (with retail employment dropping to zero in 2009) due to recession and are in the midst of a recovery but with the treatment corridor lagging in terms of new employment growth. In terms of food/accommodation sector, both show pre-constructions drops in employment with rebounds in employment in the years after. Given the proximity of the corridors, this is to be expected and is reflected by the non-significant t-test results. 

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9.5, fig.asp=.5}

mass_agg <- agg_trend_table(indy_corridor, group = 2)
agg_retail_trend_plot <- agg_trend_plot(mass_agg, industry = "Retail", industry_code = "CNS07", corridor_name = "Massachusetts Ave.", construct_year = 2009, end_year = 2010)

agg_food_trend_plot <- agg_trend_plot(mass_agg, industry = "Food/Accommodataion",industry_code = "CNS18", corridor_name = "Massachusetts Ave.", construct_year = 2009, end_year = 2010)

plot_grid(agg_retail_trend_plot, agg_food_trend_plot)
```

What follows is a plot of accommodations employment growth indexed to the construction starting year 2009. We do not include the retail index numbers because there was no 0 retail employment in the treated section of Massachusetts, according to the LEHD, so there is plot for retail. The food/accommodations index plot does show the rather vigorous employment growth on the greater Massachusetts Ave corridor. The growth is all the more impressive when compared to the city as a whole. While the Massachusetts corridor is starting from a smaller base the growth is still impressive, but does not really start until well after construction of the new cycling lanes. 
...
# mass_city_idx_retail_plot <- city_agg_index_trend_one_control_plot(df_plot = mass_index, industry = "Retail", corridor_name = "Massachusetts Ave.", control_corridor = "Massachusetts Ave.", industry_code = "CNS07_sd", construct_year = 2009, end_year = 2010)

# mass_city_idx_accom_plot <- city_agg_index_trend_one_control_plot(df_plot = mass_index, industry = "Accommodations", corridor_name = "Massachusetts Ave.", control_corridor = "Massachusetts Ave.", industry_code = "CNS18_sd", construct_year = 2009, end_year = 2010)
  
mass_index <- mass_index %>% mutate(Type = if_else(Type == "city", "City", Type))


mass_index$Type <- factor(mass_index$Type,
                        levels = c("Treatment","City",
                                   "Control: Mass Ave"))

construct_year <-  as.Date("2009-01-01","%Y-%m-%d")
end_year <- as.Date("2010-01-01","%Y-%m-%d")


mass_city_idx_retail_plot <- ggplot(mass_index, aes(x = as.Date(year, "%Y"), y = CNS07_sd,
                                group = Type, colour = Type, shape=Type)) +
    ...
    scale_x_date(date_breaks = "3 years", date_labels = "%Y") +
    theme_minimal() +
    labs(title = "Retail Employment Comparison:\n Massachusetts Ave.", x="Year",y="Employment Index",
         caption = glue("Employment is indexed to 2009\n Shaded Area is Construction Period")) +
    guides(title = "Street Type")

mass_city_idx_accom_plot <- ggplot(mass_index, aes(x = as.Date(year, "%Y"), y = CNS18_sd,
                                group = Type, colour = Type, shape=Type)) +
    geom_rect(aes(xmin = as.Date("2009-01-01","%Y-%m-%d"),
                  xmax = as.Date("2010-01-01","%Y-%m-%d"),
                  ymin = -Inf, ymax = Inf),
    ...
    scale_x_date(date_breaks = "3 years", date_labels = "%Y") +
    theme_minimal() +
    labs(title = "Food/Accommodations Employment Comparison:\n Massachusetts Ave.", x="Year",y="Employment Index",
         caption = glue("Employment is indexed to 2009\n Shaded Area is Construction Period")) +
    guides(title = "Street Type")

plot(mass_city_idx_accom_plot)
...
We did not run a DiD model for the Massachusetts Ave corridor because it violated too many assumptions of the model.
...
### Virginia and Massachusetts Avenue
